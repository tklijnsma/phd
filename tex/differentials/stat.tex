...
% 
Following the prescription in Ref.~\cite{Hansen:LShape} it is attempted to suppress high-frequency contributions originating from amplified statistical fluctuations while preserving the natural fluctuations of the measurement.
% 
It is found that no particular regularization strength is able to separate these two sources of fluctuations, and as such applying any regularization cannot be motivated.
% 
...
% 
\item $n^{\text{OOA},\,km}_i(\vec{\theta})$ is the number of events reconstructed in bin $i$ originating from outside the fiducial phase space (see Section~\ref{sec:inputs}), and $f^{\text{OOA},\,km}( \mathcal{O} | \vec{\theta} )$ is the distribution thereof.
% 
\end{itemize}
% 
...
\begin{linenomath*}
\begin{equation}
\int \mathrm{d} \mathcal{O} \, S_i^{km}( \mathcal{O} | \vec{\Delta\sigma}, \vec{\theta})
    = n_i^{\text{sig},\,km}(\vec{\Delta\sigma} | \vec{\theta})
      + n^{\text{OOA},\,km}_i
\,.
\end{equation}
